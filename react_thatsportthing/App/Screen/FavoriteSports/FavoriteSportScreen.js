  ...
  onPressAction = (data, key) => {
      ...
      this.state.data.map(item => {
        if (item.id == key) {
          ...
          if (item.check === true) {
            if (this.state.SelectedFakeContactList.length < 6) {
              this.state.SelectedFakeContactList.push(item);
              this.setState({
                selected: this.state.SelectedFakeContactList.length
              });
              console.log(this.state.SelectedFakeContactList.length);
            } else {
              item.check = !item.check;
              alert("You can select sport up to 6");
            }
  ...
  renderPictures(data, index) {
    return (
      <View style={Liststyles.GridViewBlockStyle}>
        <TouchableOpacity onPress={() => this.onPressAction(data, data.id)}>
          <View
            style={{
              alignItems: "center",
              alignContent: "center",
          ...
          >
            <View
              style={{
                width: 55,
                height: 55,
                borderColor: data.check ? Colors.red : Colors.transparent,
                borderWidth: 3,
                borderRadius: 27.5,
                alignContent: "center",
                justifyContent: "center",
